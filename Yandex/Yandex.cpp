...
private:
    ...
    double СountIDF(string plus_word) const {
        return log(double(number_documents_) / word_to_document_freqs_.at(plus_word).size());
    }
    ...
    vector<Document> FindAllDocuments(const Query& query_words) const {
        ...
        for (const auto& plus_word : query_words.plus_words) {
            if (word_to_document_freqs_.count(plus_word) == true) {                          //если +слово найдено среди слов документа тогда считаем окончательную релевантость
                for (const auto& pair_word : word_to_document_freqs_.at(plus_word)) {

                    ;
                    document_to_relevance[pair_word.first] += (СountIDF(plus_word) * pair_word.second);
                }
            }
        }
        ...
        for (const auto& minus_word : query_words.minus_words) {
            if (word_to_document_freqs_.count(minus_word) == true) {
                for (const auto& pair_word : word_to_document_freqs_.at(minus_word)) {
                    document_to_relevance.erase(pair_word.first);
                }
            }
        }
